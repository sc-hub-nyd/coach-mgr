    ...
    if (filteredMenus.length === 0) {
        ...
        libraryList.innerHTML = sortedCategories.map(cat => {
            ...
            const cardsHtml = menus.map(m => {
                ...
                return `
                <div class="c-library-card c-card card" >
                    <div>
                        <div class="c-library-card__header">
                            <div>
                                <span class="c-status c-status--muted">${cat}</span>
                            </div>
                            <div class="c-library-card__actions c-action-group c-action-group--end c-action-group--compact">
                                ${actionBtns}
                            </div>
                        </div>
                        <div class="c-library-card__title" >
                            ${escapeHtml(m.focus)}
                            ${m.engagement ? `<span class="c-library-card__rating" >${'★'.repeat(m.engagement)}${'☆'.repeat(5 - m.engagement)}</span>` : ''}
                        </div>
                    ...
                    <details class="c-content-disclosure c-content-disclosure--soft" >
                        ...
                        <div class="c-content-disclosure__body"  onclick="event.stopPropagation();">
                            ...
                            ${m.videoUrl ? `<div><strong class="c-content-disclosure__label" ><i class="u-ext-16 fa-brands fa-youtube" ></i> 参考動画</strong><div class="c-content-disclosure__text" ><a class="c-content-disclosure__link" href="${escapeHtml(m.videoUrl)}" target="_blank" rel="noopener noreferrer" ><i class="u-ext-33 fa-solid fa-arrow-up-right-from-square" ></i> 参考動画を見る (YouTube)</a></div></div>` : ''}
            ...
            <div class="c-section-group category-section" >
                <h3 class="c-section-group__title" >
                    ${cat} <span class="c-section-group__count" >(${menus.length}件)</span>
                </h3>
