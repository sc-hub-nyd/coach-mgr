...
`components.css`に残る既存規則は、見た目が古いという理由だけで削除しない。波6以降の削除では、`scripts/audit-legacy-css.sh`を実行して`reports/legacy-css-audit.tsv`を更新し、対象セレクタがCSS以外のプロジェクトファイルから参照されていないことを確認する。静的参照がある規則は、対応するHTMLまたはJavaScriptを`c-*`部品へ移行する変更と同じ公開単位で置換・削除する。

| 判定 | 許可される扱い | 禁止される扱い |
|---|---|---|
| 静的な非CSS参照が0件 | DOM契約・機能スモーク・P33/P34/P35・レスポンシブ検証を通過後に、独立コミットで削除する。 | 監査結果だけを根拠に、canvasやライブ記録の実行時挙動を無確認で削除する。 |
| 静的な非CSS参照がある | 置換先部品、DOM ID、イベント接続を確認し、移行と同時に処理する。 | 参照がある規則を一括削除する。 |
...
波6ではこの手順により、静的参照0件が確認できた39個の`u-ext-*`規則を削除し、監査対象の未使用候補を0件とした。残る`u-ext-*`規則は使用中であり、次の変更時に意味のある`c-*`または画面固有クラスへ移す。
