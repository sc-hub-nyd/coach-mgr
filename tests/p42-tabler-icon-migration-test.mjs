...
const [index, serviceWorker, tablerCss, iconSystem, componentsSystem, dashboard, ...runtimeSources] = await Promise.all([
    read('index.html'),
    read('sw.js'),
    read('assets/vendor/tabler-icons/tabler-icons-subset.css'),
...
await Promise.all([
    ...
    access(path.join(root, 'assets/vendor/tabler-icons/subset-unicodes.txt')),
    access(path.join(root, 'assets/vendor/tabler-icons/fonts/tabler-icons-subset.woff2'))
]);
...
assert.match(index,
    /assets\/vendor\/tabler-icons\/tabler-icons-subset\.css\?v=3\.46\.0-subset\.\d+/,
...
assert.match(serviceWorker,
    ...
    'TablerサブセットCSSをService Workerのprecacheへ含める必要があります');
assert.match(serviceWorker,
    /'\.\/assets\/vendor\/tabler-icons\/fonts\/tabler-icons-subset\.woff2'/,
    'TablerサブセットWOFF2をService Workerのprecacheへ含める必要があります');
assert.doesNotMatch(serviceWorker,
    /'\.\/assets\/vendor\/tabler-icons\/tabler-icons\.css'|'\.\/assets\/vendor\/tabler-icons\/fonts\/tabler-icons\.woff2'/,
    '未使用のフルTabler資産をPWAのprecacheへ残してはいけません');
assert.doesNotMatch(serviceWorker, /font-awesome/,
    'Service Workerの外部キャッシュ対象へFont Awesomeを残してはいけません');

assert.match(tablerCss, /Tabler Icons 3\.46\.0/, '固定版Tabler Icons 3.46.0のサブセット生成情報を維持する必要があります');
assert.match(tablerCss, /Generated by scripts\/build-tabler-subset-v13085\.mjs/, 'サブセットCSSは再生成可能なスクリプト由来である必要があります');
