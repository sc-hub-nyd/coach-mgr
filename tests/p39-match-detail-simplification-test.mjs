...
assert.match(source, /renderPeriodTimelineList\(period, feedbackMemoId = ''\)/);
assert.match(source, /is-pulse-timestamp-settle/);
assert.match(source, /updateData\(\{ rerender: true, feedback: true \}\)/);
assert.match(systemCss, /\.period-timeline-edit__seconds/);
assert.match(systemCss, /\.btn-use-current-timestamp/);
assert.match(systemCss, /\.timeline-edit-row\.is-pulse-timestamp-settle/);
assert.match(systemCss, /color-motion-complete/);
