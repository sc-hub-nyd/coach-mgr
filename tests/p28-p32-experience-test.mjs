...
assert.match(app, /dash-myplayer-profile-trigger/, 'マイ選手詳細を開く操作には専用トリガーが必要です');
...
assert.match(app, /id="dash-btn-open-myplayer-detail"/, 'マイ選手の表示領域には詳細を開く操作が必要です');
assert.match(app, /navigate\('player-detail', \{ playerId: player\.id, returnRoute: 'dashboard', source: 'parent-my-player' \}\)/, '保護者マイ選手の詳細はダッシュボード復帰コンテキストを持つ必要があります');
assert.match(app, /current === 'player-detail' && detailParams\.returnRoute === 'dashboard'/, '保護者マイ選手の戻る先はダッシュボードに固定する必要があります');
