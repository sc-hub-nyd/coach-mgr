...
assert.match(serviceWorker, /coachmgr-v210/, 'PWA更新シナリオは現在のキャッシュ世代をprecacheする必要があります');
...
assert.match(tokens, /--nav-floating-blur:\s*blur\(1\.7rem\) saturate\(188%\) brightness\(1\.03\);/, 'ライトモードの浮遊ナビは反射を強める専用blurトークンを持つ必要があります');
