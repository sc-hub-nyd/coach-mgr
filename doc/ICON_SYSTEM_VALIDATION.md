...
本書は、CoachMgrのアイコンを単なる視覚装飾ではなく、操作・状態・戦術表現を補助するデザインシステム部品として維持するための検証基準である。対象は、ローカルTabler Icons、絵文字、Canvas作図、テーマ適合性、PWAオフライン配信、アクセシビリティである。
...
| 層 | 実装 | 担当する意味 | 検証対象 |
|---:|---|---|---|
| 1 | ローカルTabler Icons 3.46.0のサブセットCSS／WOFF2と`.ti` | 主要ナビ、操作、試合記録、サッカー補助語彙 | ローカル定義、未定義クラス、再生成、Font Awesome再導入禁止 |
| 2 | 可視テキストラベル | 画面名、操作名、状態 | アイコンと同じ意味、単独操作の`aria-label` |
...
Tablerに意味の一致する笛やサッカー固有の警告・退場アイコンがない場合は、曖昧な汎用アイコンで代用しない。候補をTablerサブセットへ追加する前に、同じ台帳・テーマ・PWA契約との整合を確認する。
