...
export const APP_VERSION = 'v1.31.49';
...
export const RELEASE_NOTES = [
    {
        version: 'v1.31.49',
        date: '2026-08-22',
        title: 'Pitchside Pulse U4〜U5：保存・時刻・作図へ短い接地感を拡張',
        features: [
            '画面の連続性：ダッシュボード、試合、練習、空状態を含む画面切替へ、到着を補助する短いopacity／transform表現を追加。練習を保存・更新した後は、一覧先頭ではなく保存対象のカードだけを成功状態で短く着地',
            '動画分析：タイムラインの秒数を編集または現在時刻から反映した後、該当する編集行だけへ成功状態の着地を追加。並び替え・再描画後も対象記録を維持',
            '作図：選択、選手・ボール等の単点配置、線・図形の確定時にCanvasデータを変えず、Canvasラッパーだけへ短い接地感を追加。Undo／Redo、ヒットテスト、描画順、保存形式を維持',
            '統制：P39、P40、P45へ動画分析・Canvas・保存対象カードの再発防止契約を追加。保護者閲覧専用、縮退モーション、直接色ゼロ、PWAを含む全47件の契約テストを確認'
        ]
    },
    {
        version: 'v1.31.48',
        date: '2026-08-22',
        title: 'Pitchside Pulse U1〜U3：状態色とモーションで選手履歴の手応えを追加',
        features: [
            '共通基盤：チームカラーをブランド・主操作に保ったまま、年表の経路と保存完了には成功色、検索復帰には警告色を使うセマンティックなモーショントークンを追加。ライト／ダークと縮退モーションの両方で同じ意味を維持',
            '移動の連続性：実際のルート変更時だけ、選択中のモバイルボトムナビへ短い到着反応を追加。既存のタブ、モーダル、コンテキストバーのトークン駆動モーションと整合',
            '年表の手応え：学年・月の展開に経路表現、検索復帰に一度だけの到着表現、育成ノート保存に新規記録を開いた成功着地を追加。保護者の閲覧専用、キーボード、フォーカス復元を維持',
            '統制：P19へ状態トークン、ナビゲーション到着、検索復帰、保存完了の再発防止契約を追加。P37、P38、P45、P53を含む全47件の契約テストを確認'
        ]
    },
    {
        version: 'v1.31.47',
        date: '2026-08-22',
        title: '選手履歴を「探せる出場経験」と事実ベースの節目へ拡張',
        features: [
            '年表の読みやすさ：育成年表の内側スクロールを撤去し、開いている月を通常のページフローで閲覧できるように変更。検索結果が多い場合は20件ずつ明示的に追加表示',
            '構造化検索：画面内の「記録を探す」に年度、次の一歩あり、スキル評価ありの条件を追加。検索条件と段階表示件数は選手ごとのセッション状態として復元',
            '出場の足あと：直近5試合、年度・相手・結果・得点・アシストで絞り込める年度横断の試合探索を追加。過去年度の試合を選ぶと該当ワークスペースへ切り替えて既存の試合詳細を開く',
            '経験の節目：初出場、初得点、初アシスト、記録の多い月を保存データから導出して表示。順位、連続達成、報酬やバッジは導入せず、競争性を持たない振り返りに限定',
            'PWA・統制：新しい出場経験サービスをoffline precacheへ追加し、P19、P22、P35を拡張。全47件の契約テストを確認'
        ]
    },
    {
        version: 'v1.31.46',
        date: '2026-08-22',
        title: '選手の成長年表を年度横断・月間探索・役割別フォーカスへ拡張',
        features: [
            '年度横断：全年度ワークスペースから同一選手のノート、観察、試合、練習を集約し、年度コピーされたノート・観察メモは正本だけを表示。過去年度の試合詳細も正しいワークスペースへ切り替えて開く',
            '探索体験：選手ごとに年表／検索モード、学年・月・記録の開閉、検索語・種別、検索復帰位置をセッション中に保持。現在地パンくず、検索結果への復帰、検索到達記録の強調、モードタブのキーボード操作を追加',
            '月間・学年俯瞰：月ごとの種別件数と「今月のひとこま」、前後の実在記録月への移動、学年別の代表テーマ・種別件数を追加し、記録量が増えても見通しを保つ',
            '役割別表示：保護者には選択月の振り返りを提示し、コーチには前回の探索条件、画面内検索、未記録月への育成ノート追加を提供。編集導線はコーチだけに限定',
            'PWA・統制：新しいタイムラインサービスをoffline precacheへ追加し、P19・P22を拡張。P37、P42、P47、P53、P55を含む全47件の契約テストを確認'
        ]
    },
    {
        version: 'v1.31.45',
        date: '2026-08-22',
        title: '選手詳細を「成長の年表」と画面内検索へ統合',
        features: [
            '年表：育成ノートを学年、月、記録詳細の順にたどるルート型のドリルダウンへ変更。現在の学年・最新月を開き、過去の章は必要なときに開ける構成へ整理',
            '検索：サイドバーやモーダルではなく、選手詳細内の「記録を探す」コンテンツモードとして提供。検索結果から該当する学年・月・記録詳細を開いた年表へ戻れるように改善',
            'アクセシビリティ：学年・月・記録を標準button、aria-expanded、aria-controlsで操作可能にし、保護者モードの閲覧専用とコーチの削除権限を維持',
            'アイコン・表現：カスタムSVGマスク、data URIの曲線、インライン操作を削除し、ローカルTabler Iconsとセマンティックトークンへ統一',
            '統制：P19へ年表・画面内検索・検索復帰・Tabler限定・カスタムSVG非使用の再発防止契約を追加'
        ]
    },
    {
        version: 'v1.31.27',
        date: '2026-08-21',
        title: '育成ノートの「成長の年表」UIリファクタリング',
        features: [
            'デザイン：育成ノートをタイムライン表示へ刷新し、種別ごとの色分けドットと縦のコネクタで「成長のつながり」を表現',
            '検索・絞り込み：年表上部にインライン検索バーと種別フィルターを新設し、画面遷移なしで即座に絞り込み可能に',
            '入力フォーム分離：記録フォームをモーダルへ分離し、画面右下のフローティングボタン(FAB)からいつでも入力できるよう改善',
            '権限制御：保護者モードではFABを非表示にし、引き続き閲覧専用の安全なUIを提供'
        ]
    },
    {
        version: 'v1.31.26',
        date: '2026-08-21',
        title: '選手詳細の累積KPI表記を「通算」へ統一',
        features: [
            '表記統一：年度をまたぐ累積値である試合出場、得点、アシストの補足を「通算」にそろえ、「全期間」との混在を解消',
            '情報設計：年度出席率は年度内の試合・練習を対象とする値として維持し、年度値と通算値の区別を明確化',
            '統制：P36で3つの累積KPIの「通算」表記と、「全期間」不使用を契約化',
            '互換性：既存の統計集計、KPI ID、保護者閲覧、デスクトップ・モバイル表示、PWAオフライン動作を維持'
        ]
    },
    {
        version: 'v1.31.25',
        date: '2026-08-21',
        title: '選手詳細KPIのモバイル表示を1行4列へ最適化',
        features: [
            'スマホ表示：年度出席率、試合出場、通算得点、通算アシストを均等な4列の1行へ固定し、一覧性を向上',
            '可読性：標準c-metricのsurface・アイコン・数値表現を維持しながら、モバイルだけ余白とアイコン面をコンパクト化。ラベルの省略と補足非表示で折返しを防止',
            '互換性：既存のKPI ID、統計集計、保護者閲覧、デスクトップレイアウト、キーボード操作、PWAオフライン動作を維持',
            '統制：選手詳細専用モディファイアを追加し、P36でモバイル4列と補足テキストの折返し防止を契約化'
        ]
    },
    {
        version: 'v1.31.24',
        date: '2026-08-21',
        title: '選手詳細のKPIと育成ノートを標準部品へ統一',
        features: [
            'KPI：年度出席率、試合出場、通算得点、通算アシストを未定義の個別カードから標準c-metricへ移行し、共通surface、アイコン面、余白、数値書式を統一',
            '情報階層：出席率は試合・練習を対象とする年度値、出場・得点・アシストは通算／全期間であることを補足して集計範囲を明示',
            '育成ノート：廃止済みタブ用クラスを除去し、成長サマリー、スキル推移、履歴、記録フォームを通常の縦セクションと共通focus summaryへ統一',
            '統制：P36へ標準KPI、旧c-metric-card不使用、育成ノートの可視縦セクション、旧タブCSS削除の契約を追加',
            '互換性：選手別の統計集計、育成ノート記録・削除、保護者閲覧専用、詳細への戻る、PWAオフライン動作を維持'
        ]
    },
    {
        version: 'v1.31.23',
        date: '2026-08-21',
        title: '選手統計・一覧カードとスマホ作図シーン列を統一',
        features: [
            '選手統計：ダッシュボードと選手詳細の年度出席率、通算得点・アシスト、出場試合を共有サービスへ統一し、presentPlayerIdsと旧attendedPlayerIds、試合直下と旧フォーメーション内の得点記録を互換的に集計',
            '出場履歴：フォーメーション配置または参加記録がある試合を表示し、参加済みでも一覧に出ない記録漏れを解消。アシストの単位を回へ統一',
            '選手管理：ポジション、氏名、プレースタイル、強み、フォーカスを固定行グリッドへ再構成し、ポジション数や未設定値でカード内の情報位置・高さが揺れる問題を解消',
            'スマホ作図：シーン列の横スクロールを内側リストへ一本化し、最終シーンの後ろに残る不要な予約領域を除去。右端まで自然に表示・操作可能に改善',
            '互換性：既存・旧参加記録、フォーメーション、得点記録、PWAオフライン、キーボード操作、Canvas・アニメーション操作を維持'
        ]
    },
    {
        version: 'v1.31.22',
        date: '2026-08-21',
        title: '選手詳細の閲覧専用権限とデザイン契約を強化',
        features: [
            '保護者モード：選手詳細の編集・削除、観察メモ追加、評価編集・削除、育成ノート記録・削除をUIと処理の両方で禁止',
            '権限保護：旧IDを参照していた閲覧専用CSSを現行の選手詳細アクションIDへ同期し、直接操作・古いDOM経由でもコーチ権限を再確認',
            'デザインシステム監査：共通カード、KPI、セクション、アクション群、セマンティックトークンの適用を確認。閲覧・分析導線は維持',
            '互換性：コーチモードの選手編集・削除、保護者の出席・得点・アシスト・フィードバック・試合履歴・作図閲覧を維持'
        ]
    },
    {
        version: 'v1.31.21',
        date: '2026-08-21',
        title: '保護者マイ選手の詳細導線とタイトル配置を統一',
        features: [
            '保護者ダッシュボード：指定済みマイ選手の表示領域を選手詳細への操作部品へ変更。詳細画面の戻る先は常にダッシュボード',
            'アクセシビリティ：マイ選手詳細の操作に名前を含む説明、キーボードフォーカス、十分なタップ領域を追加',
            'デザイン：ページ、セクション、モーダル、ダッシュボード、カード、選択シートの共通タイトルを中央揃えへ統一。本文・フォーム・一覧行の可読性は左揃えを維持',
            '互換性：コーチモードと通常の選手詳細は既存の履歴復帰を維持し、保護者マイ選手の出席・得点・アシスト確認導線も維持'
        ]
    },
    {
        version: 'v1.31.20',
        date: '2026-08-21',
        title: 'モバイル詳細導線とマイ選手・作図レイヤーを改善',
        features: [
            'スマホ動画分析：左上の戻るを画面下部の共通コンテキストバーへ統一し、試合詳細への安全な復帰処理を共有',
            'スマホ作図・動画分析：ボトムナビ非表示時の予約領域を取り除き、Safe Areaと下部戻るバーに必要な余白だけを確保',
            '保護者ダッシュボード：指定済みマイ選手の要約、最新フィードバック、参加・得点・アシスト確認を中心導線とし、重複する詳細プロファイルを削除',
            '作図：円・四角を常に背面へ描画し、選手・ボール・マーカー・矢印などの前景オブジェクトを優先選択',
            '互換性：保存済み作図、Undo／Redo、図形の選択・移動・リサイズ、動画停止、タイマー解除、アクセシビリティ、PWAオフライン動作を維持'
        ]
    },
    {
        version: 'v1.31.19',
        date: '2026-08-21',
        title: 'PCユーティリティとスマホ設定・作図の操作導線を改善',
        features: [
            'PC：左サイドバー下部を役割、テーマ、クラウド同期、バージョンの順へ整理し、役割行を固定ラベル「役割」と状態が分かる二択スイッチへ統一',
            'スマホ設定：カテゴリタブをフローティングボトムナビの直上に固定し、スクロール位置にかかわらず設定カテゴリを切り替え可能に改善',
            'スマホ作図：左上の専用戻るを共通コンテキストバーへ統一。未保存確認と作図元に応じた戻り先を維持し、単一シーン時の右側余白を解消',
            '互換性：既存DOM ID、キーボード操作、ARIA、テーマ切替、Canvas操作、PWAオフライン動作を維持'
        ]
    },
    {
        version: 'v1.31.18',
    ...
    },
    {
        version: 'v1.31.16',
        date: '2026-08-21',
        title: 'チーム固有の名称・資産参照を汎用化',
        features: [
            '識別子：ソース、設定、テスト、資料、スライドから特定チーム名・略称・外部ブランド参照を除去',
            '資産：カスタムSVGを汎用チームアイコンとして再配置し、CSSマスク、PWA precache、既存の意味クラスを維持',
            '設計：既定赤系はアプリ固有のフォールバックとして維持し、任意チームカラー・動的テーマ・保存済みテーマの優先規則を変更しない',
            '統制：固有チーム識別子・パス名の再導入を防ぐP55横断契約を追加'
        ]
    },
    {
        version: 'v1.31.15',
    ...
    },
    {
        version: 'v1.31.13',
        date: '2026-08-21',
        title: 'PCサイドバー下部のユーティリティ配置を縦並びへ整理',
        features: [
            'UI：PC左サイドメニュー下部を、モード、クラウド同期、表示テーマ、バージョンの一貫した縦並びへ再構成',
            'レイアウト：モード・同期・表示テーマを同じ最小行高と左右整列にそろえ、テーマスイッチ追加後の横並び・二行化による視覚バランスの崩れを解消',
            '互換性：既存のモード切替、クラウド同期ポップオーバー、Light／Dark切替、キーボード操作、モバイルその他メニューを維持',
            '検証：P38へPCサイドバーユーティリティの表示順と行高整合性の再発防止契約を追加'
        ]
    },
    {
        version: 'v1.31.12',
    ...
    },
    {
        version: 'v1.31.10',
        date: '2026-08-21',
        title: 'スマホ作図のタブ・ツール・詳細の表示順を固定',
        features: [
            '不具合修正：モバイル作図でCanvas領域の内部にあったタブ・操作群をワークスペースの同一flex列として扱い、メニュー詳細がツールドックより先に表示される問題を解消',
            'レイアウト：描画モードはピッチ→描画ツールタブ→ツールドック→メニュー詳細、アニメーションモードはピッチ→アニメーションタブ→シーン列→プレイヤー操作→詳細の順を固定',
            'モバイル作図のピッチ領域は8:5比率を維持し、操作群や詳細の順序変更によってCanvasの表示位置が不安定にならないよう調整',
            '検証：P40へモバイル作図の要素順契約を追加し、既存のCanvas・タッチ操作・アニメーション回帰契約を維持'
        ]
    },
    {
        version: 'v1.31.9',
        date: '2026-08-20',
        title: 'アニメーション動画出力合成・テロップ再生連携＆メニュー・戦術個別JSON共有',
        features: [
            '不具合修正：動画ダウンロード時に背景ピッチ（緑芝・ライン）と選手・矢印・シーン見出し・指導テロップを完全合成して出力するよう改修（動画背景が黒くなる問題を解消）',
            '不具合修正：アニメーション再生時にピッチキャンバス中央下部にシーン見出しおよび指導テロップを高コントラストオーバーレイで常時表示',
            '新機能：メニューライブラリおよび戦術管理の一覧に個別エクスポート（ダウンロード）およびインポート（取込）機能を追加。他学年や外部コーチとのデータやり取りを1クリックで実現',
            '検証：全45件の自動契約テストを実行し、全パスを確認',
            '利用者操作不要'
        ]
    },
    {
        version: 'v1.31.8',
        date: '2026-08-20',
        title: 'スマホ描画ツールの2段配置（横スクロール廃止）＆タブ切替時のピッチ位置安定化',
        features: [
            '機能・UI変更：スマホ表示時の描画ツールを2段グリッド配置（全14種を7列×2行）に変更し、横スクロールなしで全ツールに即座にアクセス可能に改善',
            'レイアウト最適化：描画ツールタブとアニメーションタブの下部占有高さを122pxで完全統一。タブを切り替えてもピッチキャンバスのサイズや位置が一切ブレない安定構造を実現',
            '機能統合：シーン詳細（見出し・テロップ・停止時間）の編集UIをボトムシート（ドロワー）に完全統一し、モーダルとの二重表示を解消',
            'UI改善：PC・スマホ共通でタイムラインカードの巨大化を解消し、コンパクトなピル型カードで直感的な操作感を提供',
            'UI改善：PC版アニメーションツールバーの固定高さを緩和し、ボタンのはみ出し・見切れを解消',
            '検証：全45件の自動契約テストを実行し、全パスを確認',
            '利用者操作不要'
        ]
    },
    {
        version: 'v1.31.7',
        date: '2026-08-20',
        title: 'スマホレイアウトでのタブ浮遊およびPCでのメニュー見切れ不具合の修正',
        features: [
            '不具合修正：スマホ表示時に「描画ツール / アニメーション」タブがピッチ中央に浮いてしまうレイアウト構造を修正し、画面下部へ正常に配置',
            '不具合修正：PC表示時にアニメーションメニュー（タイムライン等）がピッチ領域に重なりすぎて作図領域を圧迫していた問題を修正し、専用の余白（下部）へ配置するよう改善',
            '検証：全45件の自動契約テストを実行しパスを確認。PCおよびスマホ双方での操作性に影響がないことを確認',
            '利用者操作不要'
        ]
    },
    {
        version: 'v1.31.6',
        date: '2026-08-20',
        title: 'スマホレイアウトでの一部オブジェクト（ツール・選手）消失・見切れ問題の修正',
        features: [
            '変更概要：スマホ表示時にツールドックやキャンバスが見切れるレイアウト不具合を修正',
            '機能・UI変更：スマホ用下部ドックに横スクロールバーを表示し、隠れたツールアイコンの存在を明示',
            '不具合修正：ピッチのアスペクト比指定の競合を解消し、選手が画面外へ押し出される縦潰れ現象を解消',
            '不具合修正：スマホ版のカラー変更・一括配置ポップオーバーが切り取られないよう上部展開へ変更',
            '検証：全45件の自動契約テストを実行し、レスポンシブ崩れが解消されたことを確認',
            '利用者操作不要'
        ]
    },
    {
        version: 'v1.31.5',
        date: '2026-08-20',
        title: '作図・アニメーション画面のレスポンシブ再設計（PC案1＋スマホ案1）',
        features: [
            'PC・大画面（幅769px以上）: 左側ツールパレットを56pxスリム1列ドックへ刷新し、ピッチ領域を最大化',
            'スマホ・小画面（幅768px以下）: 左サイドバーを撤廃し、画面最下部へ横スクロール可能なボトムドックとして集約',
            'スマホ専用の操作モード切替タブ（「描画ツール」⇄「アニメーション」）を新設し、親指操作と全幅ピッチ表示を両立',
            '全45件の自動契約テストおよびJavaScript構文チェックの完全パスを検証'
        ]
    },
    {
        version: 'v1.31.4',
        date: '2026-08-20',
        title: '作図ツールドック2列化・アニメーションパレット最適化＆透過解消',
        features: [
            '作図ツールドック（オブジェクトパレット）を2列グリッド（132px幅）へ再設計し、画面下部の見切れを解消して全ツールを一目で選択可能に改善',
            'アニメーションパレット（シーンタイムラインと再生コントロールバー）の重なりと余白を最適化し、ピッチ下部の視認性と操作性を大幅向上',
            'CSS変数--surfaceのエイリアス定義により、作図・動画分析サイドメニューの背景透過問題を解消',
            '全45件の自動契約テストの完全パスを検証'
        ]
    },
    {
        version: 'v1.31.3',
    ...
    },
    {
        version: 'v1.22.0',
        date: '2026-08-18',
        title: '既定チームブランド・デザインシステム正式統合',
        features: [
            '既定チーム公式サイトの赤系を既定ブランド種色とするカラー階層、任意色拡張、既存チーム保護、禁止事項を標準仕様として文書化',
    ...
    },
    {
        version: 'v1.21.0',
        date: '2026-08-18',
        title: '既定チーム公式サイト基準の赤系ブランドテーマ',
        features: [
            '新規・未設定チームの既定種色を、既定チーム公式サイトのヒーロー表現を基準とする赤系 #EF3340 へ変更。設定画面からの任意色変更は継続',
    ...
    },
    {
        version: 'v1.20.0',
        date: '2026-08-18',
        title: '既定チームアイコンシステム統合',
        features: [
            '既定チーム提供の44個SVGをカテゴリ・ファイル名を維持してデザインシステム資産へ登録。CSSマスクとcurrentColorでlight/dark・チームテーマへ適応',
            'チームシグナル、ホーム、試合、練習、振り返り、戦術、選手の主要ナビゲーションを既定チームアイコンへ段階移行',
